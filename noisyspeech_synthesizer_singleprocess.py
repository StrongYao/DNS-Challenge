...
def main_body():
    ...
    utils.write_log_file(
        log_dir, "low_activity_files.csv", clean_low_activity_files + noise_low_activity_files
    )

    # Compute and print stats about percentange of clipped and low activity files
    total_clean = (
        len(clean_source_files) + len(clean_clipped_files) + len(clean_low_activity_files)
    )
    total_noise = (
        len(noise_source_files) + len(noise_clipped_files) + len(noise_low_activity_files)
    )
